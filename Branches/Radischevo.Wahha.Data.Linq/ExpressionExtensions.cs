...
namespace Jeltofiol.Wahha.Data.Linq
{
    public static class ExpressionExtensions
    {
        ...
        private static void ConvertExpressions(ref Expression expression1, ref Expression expression2)
        {
            ...
            {
                var isNullable1 = expression1.Type.IsNullable();
                var isNullable2 = expression2.Type.IsNullable();
                if (isNullable1 || isNullable2)
                {
                    if (expression1.Type.MakeNonNullableType() == 
                        expression2.Type.MakeNonNullableType())
                    {
                        if (!isNullable1)
                        {
